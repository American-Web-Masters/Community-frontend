    ...
    try {
      ...
      const mappedSubscriptions =
        response.data?.subscriptions?.map((sub) => ({
          ...
          recipientName:
            sub.recipientProfileId?.fullname ||
            sub.metadata?.recipientName ||
            null,
          recipientUsername:
            sub.recipientProfileId?.username ||
            sub.metadata?.recipientUsername ||
            null,
          ...
          prev.map((sub) =>
            sub.id === subscription.id
              ? {
                  ...
                  cancelAtPeriodEnd:
                    response.data.subscription.metadata?.cancelAtPeriodEnd ===
                    "true",
      ...
      {subscriptions.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-gray-600 mb-8 max-w-md mx-auto">
            You haven't set up any recurring payments yet. Start supporting your
            favourite communities with monthly donations.
          </p>
          <button
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
                  ...
                  <div className="flex space-x-3 mt-4 sm:mt-0">
                    ...
                    {subscription.subscriptionType === "community" ? (
                      <button
                        onClick={() =>
                          navigate(`/communities/${subscription.communityId}`)
                        }
                      ...
                      <button
                        onClick={() =>
                          navigate(
                            `/profile/${subscription.recipientUsername || subscription.recipientId}`,
                          )
                        }
                        className="px-4 py-2 text-blue-600 border border-blue-600 rounded-lg hover:bg-blue-50 transition-colors text-sm font-medium"
                      >
    ...
    if (receivedSubscriptions.length === 0) {
      return (
        <div className="text-center py-16">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-3">
            No Received Subscriptions
          </h3>
          <p className="text-gray-600 max-w-md mx-auto">
            Nobody has subscribed to you yet. Share your profile or communities
      ...
      <>
        ...
        <div className="flex items-center space-x-3 mb-6">
          <span className="text-sm text-gray-500">
            {receivedSubscriptions.length} subscriber
            {receivedSubscriptions.length !== 1 ? "s" : ""}
          </span>
          <span className="text-gray-300">·</span>
          <span className="text-sm text-green-600 font-medium">
            {activeReceivedCount} active
          </span>
        </div>
          ...
          {receivedSubscriptions.map((sub) => (
            ...
            >
                ...
                <div className="min-w-0">
                  ...
                  {sub.communityName && (
                    <p className="text-xs text-gray-400 mt-0.5">
                      via{" "}
                      <span className="text-gray-600">{sub.communityName}</span>
                    </p>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-2 sm:gap-1 flex-shrink-0">
                ...
                <span
                  className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(sub.status, sub.cancelAtPeriodEnd)}`}
                >
    ...
    <div className="p-6">
      ...
      <div className="flex space-x-1 bg-gray-100 rounded-xl p-1 mb-8">
        ...
        >
          ...
          {subscriptions.length > 0 && (
            <span
              className={`px-2 py-0.5 rounded-full text-xs font-semibold ${activeTab === "my" ? "bg-blue-100 text-blue-700" : "bg-gray-200 text-gray-600"}`}
            >
        ...
        >
          ...
          {receivedSubscriptions.length > 0 && (
            <span
              className={`px-2 py-0.5 rounded-full text-xs font-semibold ${activeTab === "received" ? "bg-blue-100 text-blue-700" : "bg-gray-200 text-gray-600"}`}
            >
      ...
      <ConfirmModal
        ...
        description={
          selectedToCancel
            ? `Are you sure you want to cancel your $${selectedToCancel.amount}/${selectedToCancel.interval} subscription to "${
                selectedToCancel.subscriptionType === "community"
                  ? selectedToCancel.communityName
                  : selectedToCancel.recipientName ||
                    selectedToCancel.recipientUsername ||
                    "User"
              }"? 
