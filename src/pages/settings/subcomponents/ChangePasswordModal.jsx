...
const ChangePasswordModal = ({ isOpen, onClose, onSubmit, loading = false }) => {
	...
	return createPortal(
		<div
			className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-md transition-all duration-300 animate-in fade-in ease-out flex items-center justify-center p-4 rounded-xl"
				...
				<div className="p-6">
					<div className="flex items-start justify-between gap-4">
						...
						<button
							...
							onClick={handleClose}
							disabled={loading}
							className="text-gray-500 hover:text-gray-700 text-2xl leading-none disabled:opacity-60 cursor-pointer hover:rotate-90 hover:shadow-sm active:scale-90 transition-all duration-300"
							aria-label="Close"
						>
							×
						</button>
					...
					<form onSubmit={handleSubmit} className="mt-5 space-y-4">
						...
						<div className="pt-2 flex gap-3">
							...
							<button
								type="submit"
								disabled={!canSubmit}
								className="flex-1 py-2.5 px-4 btn-blue-gradient text-white rounded-xl font-medium disabled:opacity-60 cursor-pointer shadow-sm hover:shadow-md transition-all duration-300 transform hover:-translate-y-0.5"
							>
