...
import { getStripeAccountStatus } from '../../api/communities';
...
const StripeOnboardingSuccess = () => {
  ...
  useEffect(() => {
    ...
    if (communityId) {
      checkAccountStatus(communityId);
    } else {
      setLoading(false);
    }
  }, [searchParams]);

  const checkAccountStatus = async (communityId) => {
    try {
        ...
        <div className="bg-white rounded-lg shadow-lg p-8 text-center">
          ...
          <p className="text-gray-600 mb-6">
            {accountStatus?.chargesEnabled ? (
              "Your Stripe account has been successfully connected and verified. Your community can now receive donations and support from members."
            ) : (
